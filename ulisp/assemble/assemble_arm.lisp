...
SET, ASSEMBLE, CALL, ENDFUNCTIONS };
...
typedef int (*intfn_ptr_type)(int x, int y, int z);

object *fn_assemble (object *args, object *env) {
  (void) env;
  int code, i = 0;
  ...
  while (args != NULL) {
    ...
    if (listp(arg)) {
      while (arg != NULL) {
        code = checkinteger(ASSEMBLE, first(arg));
        if (i == CODESIZE) error2(ASSEMBLE, PSTR("not enough room for code"));
        MyCode[i++] = code & 0xff;
        MyCode[i++] = (code>>8) & 0xff;
        arg = cdr(arg);
      }
    } else {
      code = checkinteger(ASSEMBLE, arg);
      if (i == CODESIZE) error2(ASSEMBLE, PSTR("not enough room for code"));
      MyCode[i++] = code & 0xff;
      MyCode[i++] = (code>>8) & 0xff;
    }
...
object *fn_call (object *args, object *env) {
  (void) env;
  int x = 0, y = 0, z = 0, w;
  if (args != NULL) { 
    x = checkinteger(CALL, first(args));
    args = cdr(args);
    if (args != NULL) {
      y = checkinteger(CALL, first(args));
      args = cdr(args);
      if (args != NULL) {
        z = checkinteger(CALL, first(args));
      }
    }
  }
  w = ((intfn_ptr_type)&MyCode[1])(x, y, z);
  return number(w);
}
...
const char string178[] PROGMEM = "set";
const char string179[] PROGMEM = "assemble";
const char string180[] PROGMEM = "call";
...
const tbl_entry_t lookup_table[] PROGMEM = {
  ...
  { string178, fn_set, 2, 126 },
  { string179, fn_assemble, 1, 127 },
  { string180, fn_call, 0, 3 },
};
...
void setup () {
  ...
  initenv();
  initsleep();
  pfstring(PSTR("uLisp 3.0x "), pserial); pln(pserial);
}
