...
defineLambda paramName body =
  return $ FunVal $ \val -> eval (Let paramName val body)
...
{-
    • Couldn't match expected type ‘Exp’ with actual type ‘Value’
      • In the second argument of ‘Let’, namely ‘val’
        In the first argument of ‘eval’, namely ‘(Let paramName val body)’
        In the expression: eval (Let paramName val body)
     |
  91 |   return $ FunVal $ \val -> eval (Let paramName val body)
     |                                                 ^^^
